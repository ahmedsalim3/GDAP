...
class ProNE(BaseNodeEmbedder):
    def __init__(
        self, n_components=32, step=10, mu=0.2, theta=0.5, exponent=0.75, verbose=True
    ):
        """
    ...
    def fit_transform(self, graph):
        ...
        """
        G = cg.csrgraph(graph)
        features_matrix = self.pre_factorization(
            G.mat, self.n_components, self.exponent
        )
    ...
    def tsvd_rand(matrix, n_components):
        ...
        U, Sigma, VT = randomized_svd(
            smat, n_components=n_components, n_iter=5, random_state=None
        )
        U = U * np.sqrt(Sigma)
