...
def predict(model, X_val, edges_val, threshold=0.5):
    ...
    protein_confidence_mapping = {
        edge: (pred.item(), confidence) for edge, pred, confidence in zip(edges_val, y_pred, confidence_scores)
    }

    # separate associated and non-associated proteins
    associated_proteins = {
        edge: (pred, confidence) for edge, (pred, confidence) in protein_confidence_mapping.items() if pred == 1
    }  # asssociated with the disease
    non_associated_proteins = {
        edge: (pred, confidence) for edge, (pred, confidence) in protein_confidence_mapping.items() if pred == 0
    }  # not associated to the disease
