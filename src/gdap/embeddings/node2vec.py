...
class Node2Vec(BaseNodeEmbedder):
    def __init__(
        ...
        keep_walks=False,
        verbose=True,
        w2vparams={"window": 10, "negative": 5, "epochs": 10, "batch_words": 128},
    ):
        """
        Parameters
        ----------
        ...
        if "size" in w2vparams.keys():
            raise AttributeError(
                "Embedding dimensions should not be set " + "through w2v parameters, but through n_components"
            )
    ...
    def fit(self, G):
        ...
        self.model = gensim.models.Word2Vec(sentences=self.walks, vector_size=self.n_components, **self.w2vparams)
