...
class BiGraph:
    ...
    def create_graph(
        ...
        # Add PPI edges
        ppi_edges_added = 0
        for _, row in _PROGRESS_BAR(ppi_df.iterrows(), total=len(ppi_df), desc="Adding PPI edges"):
        ...
        for disease in _PROGRESS_BAR(diseases, desc="Generating Negative Edges", total=len(diseases)):
            ...
            potential_negatives = ppi_df[
                ppi_df["GeneName1"].isin(non_associated_genes) | ppi_df["GeneName2"].isin(non_associated_genes)
            ]
    ...
    @staticmethod
    def visualize_sample_graph(G, ot_df, node_size=300, figsize=(20, 10), output_dir=None):
