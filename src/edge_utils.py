...
def features_labels_edges_idx(pos_edges, neg_edges, embeddings, node_index_map, scale_features=False):
    ...
    pos_edges_with_features = [((edge[0], edge[1]), edge_features_idx(edge, embeddings, node_index_map)) for edge in pos_edges]
    neg_edges_with_features = [((edge[0], edge[1]), edge_features_idx(edge, embeddings, node_index_map)) for edge in neg_edges]
...
def split_edge_data(X, y, edges, test_size=0.25):
    ...
    X_train, X_temp, y_train, y_temp, edges_train, edges_temp = train_test_split(X, y, edges, test_size=test_size, random_state=42)
