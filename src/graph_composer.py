...
class GraphComposer:
    ...
    def construct_graph(self, df: pd.DataFrame, source_col: str, target_col: str, edge_attr_col: str,
                        plot: bool = False, save: bool = False, filename: str = None, figsize: tuple = (10, 8)) -> nx.Graph:
    ...
    def compose_graphs(self):
        if self.open_target_df is not None and self.STRING_df is not None:
            self.open_target_df['datasourceScores_score'] = self.open_target_df['datasourceScores_score'].round(3) # round for better plotting
            self.open_target_G = self.construct_graph(self.open_target_df, 'datasourceScores_id', 'approvedSymbol', 'datasourceScores_score')
    ...
    def plot_community_detection(self, plot: bool = True, save: bool = False, filename: str = None, figsize: tuple = (30, 20)):
    ...
    def visualize_graphs(self, plot: bool = True, save: bool = False, filename: str = None, figsize: tuple = (30, 20)):
        if self.composed_G is None:
            raise RuntimeError("Composed graph is not available. Run compose_graphs() first.")
