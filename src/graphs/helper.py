...
def process_OT_data(api_res):
    ...
    df = pd.json_normalize(associated_targets,
                           record_path='datasourceScores',
                           meta=['target', 'score'],
                           record_prefix='datasourceScores_', errors='ignore')
...
def plot_community_detection(G, edge_attr_col, plot=True, save=False, filename=None, figsize=(30, 20)):
    if plot or save:
        ...
        nx.draw(
            ...
            node_color=colors,
            cmap=plt.cm.jet, node_size=4000, edge_color='gray', arrows=True
        )
        edge_labels = nx.get_edge_attributes(G, edge_attr_col)
        ...
        if save:
            if filename is None:
                filename = f"assets/Composed_Network_with_Communities.png"
            plt.savefig(filename)
        plt.show()
...
def visualize_graphs(G, plot=True, save=False, filename=None, figsize=(30, 20)):
    if plot or save:
        ...
        if save:
            if filename is None:
                filename = f"assets/Composed_Network.png"
            plt.savefig(filename)
        plt.show()
